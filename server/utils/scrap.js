...
const scrap = async (url) => {
    try {
        ...
        // before looking for links, click on the 'load more news' button in the web-page to obtain as many links as possible
        for (let i = 0; i < 81; i++) {
            await page.locator('.content-feed-container .load-more-button').click();
        ...
        const webElement = (".post-card-text-wrapper a")
            ...
            Object.defineProperties(article, {
                ...
                body: {
                    value: (article.body || 'N/A')
                        ...
                        .replace(/|/g, "") // semi-column ( | ) --> normal space char
                        .replace(/  /g, " "), // double space ( | ) --> normal space char
...
scrap("https://www.benzinga.com/news")
    .then(data => {
        saveToJSON(data, 'scrapedData2.json');
    });